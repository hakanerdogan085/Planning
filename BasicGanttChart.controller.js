sap.ui.define([
	...
	return Controller.extend("ZPLANING.BasicGanttChart", {
		onPress: function () {
		...
		onInit: function () {
			...
			var oData = {
				Test: {
					Ad: "",
					Baslangicsaati: "", //örnek olması için koydum
					Bitissaati: "",
					Sure: ""
				}
			};
			...
			var sPath = jQuery.sap.getModulePath("ZPLANING", "/data.json");
	...
	_handleValueHelpClose : function (evt) {
		...
		*/
		...
		firthandleDateChange: function (event) { //datetimepicker den değeri alır
			
		
			var oModel = sap.ui.getCore().getModel("oModelTest");

			var BaslangicSaati = event.getParameter("value");

			oModel.setProperty('/Baslangicsaati', BaslangicSaati);

			var a = oModel.getProperty('/Baslangicsaati');
			var b = oModel.getProperty('/Bitissaati');

			this.SureChange(a, b);

		},

		lasthandleDateChange: function (event) { //datetimepicker den değeri alır
			
			
			var oModel = sap.ui.getCore().getModel("oModelTest");

			var Bitissaati = event.getParameter("value");

			oModel.setProperty('/Bitissaati', Bitissaati);
		...
		SureChange: function (date1, date2) { //İki tarih arasında günü hesaplayan program.
			...
			if (date1 != null && date2 != null) {
				...
				var ayi = ((date1 - guni) / 100) % 100;
				var ays = ((date2 - guns) / 100) % 100;

				var yili = (date1 / 10000).toFixed(0);
				var yils = (date2 / 10000).toFixed(0);

				if (guns >= guni) {
					topgun = topgun + (guns - guni);

				} else {

					topgun = topgun + ((guns + 30) - guni);
					ays = ays - 1;
				}

				if (ays >= ayi) {
					topgun = topgun + ((ays - ayi) * 30);
				} else {

					topgun = topgun + ((ays + 12) - ayi) * 30;
					yils = yils - 1;
				}

				if (yils >= yili) {
					topgun = topgun + ((yils - yili) * 365);
				} else {
					sap.m.MessageToast.show("Yanlış giriş yaptınız.");
				}
		...
		handleName: function (event) {
			...
			var oModel = sap.ui.getCore().getModel("oModelTest");
			var Ad = event.getParameter("value");
			oModel.setProperty('/Ad', Ad);
			
			 
				var a = oModel.getProperty('/Ad');
					oModel.setData(a);
		},
		
		Kaydet: function (event){
			 sap.m.MessageToast.show("Veriler Kaydedildi");
		},
